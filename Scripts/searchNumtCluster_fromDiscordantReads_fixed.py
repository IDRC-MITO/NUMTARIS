#!/usr/bin/env python3
"""
searchNumtCluster_fromDiscordantReads_fixed.py
...
Usage:
  python3 searchNumtCluster_fromDiscordantReads_fixed.py \\
...
def count_split_support(split_df: pd.DataFrame, chrom: str, start: int, end: int, min_mapq: int):
    if split_df.empty:
        return 0
...
def main():
    ...
    for chrom, group in nuclear_df.groupby("chr"):
        ...
        for cluster_pos in clusters:
            ...
            split_support = count_split_support(
                split_df=split_df,
                chrom=chrom,
                start=start,
                end=end,
            ...
            total_support = disc_support + split_support

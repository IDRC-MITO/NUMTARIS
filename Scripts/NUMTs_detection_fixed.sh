...
#      (chrM/MT), then split them into discordant vs. split-read SAM
#      files with samblaster.
#   3. Cluster the nuclear-genome anchors of those reads and score each
#      cluster for read support via searchNumtCluster_fromDiscordantReads_fixed.py.
#
...
#
# Options (all thresholds forwarded to searchNumtCluster_fromDiscordantReads_fixed.py):
...
CLUSTERSCRIPT="${SCRIPT_DIR}/searchNumtCluster_fromDiscordantReads_fixed.py"
...
samtools view -h -F 2 "${INPUTBAM}" \
  | awk 'BEGIN{OFS="\t"} /^@/ {print; next} ($3=="chrM" || $3=="MT" || $7=="chrM" || $7=="MT") {print}' \
  | samtools sort -n - \
  | samtools view -h - \
  | samblaster \
      --ignoreUnmated \
